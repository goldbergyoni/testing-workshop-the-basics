...
test("When video instructions are valid, then a success email should be called with a whom and a what", async () => {
  // Arrange
  const clipInstructions = testHelper.factorClipInstructions({
    creator: { email: "yoni@testjavascript.com", name: "Yoni" },
    destination: "Mexico",
...
test("When video instructions are valid, then real mailer was no called", async () => {
  // Arrange
...
test("'When video instructions are valid, then real mailer was no called --anonymous spy'", async () => {
  // Arrange
  ...
  const tripClipServiceUnderTest = new TripClipService({ send: sendSpy });

  // Act
  await tripClipServiceUnderTest.generateClip(clipInstructions);

  // Assert
  expect(sendSpy.lastCall.args).toEqual([
    "yoni@testjavascript.com",
    expect.any(String),
  ]);
});
...
test("'When the VideoProducer.produce operation operation fails, an exception is thrown'", async () => {
  // Arrange
  const errorPropertyName = "video-production-failed";
  const clipInstructions = testHelper.factorClipInstructions({
    creator: { email: "yoni@testjavascript.com", name: "Yoni" },
    destination: "Mexico",
  });
  const tripClipServiceUnderTest = new TripClipService();
  sinon.stub(videoProducer, "produce").throws(errorPropertyName);
  ...
  try {
    ...
    // Assert
    expect(e.message).toContain(errorPropertyName);
  }
});
...
test("'When the InstructionsValidator class tells that the input is invalid, then the response is not succeeded'", async () => {
  ...
  // Act
  const res = await tripClipServiceUnderTest.generateClip(clipInstructions);

  // Assert
  expect(res.succeed).toBe(false);
});
